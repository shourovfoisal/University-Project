<?php
    ...
    if($result = mysqli_query($connection,$query))
    {
        ...
        if($cnt>0){
            ...
            fwrite($myfile,$tag);
            $tag = '<h3>8th semester project/thesis information</h3>';
            fwrite($myfile,$tag);
            $tag = '</div>';
            fwrite($myfile,$tag);
            ...
            fwrite($myfile,$tag);
            $tag = '<th>NUMBER OF STUDENTS(INTERNAL)</th>';
            fwrite($myfile,$tag);
            $tag = '<th>NUMBER OF STUDENTS(EXTERNAL)</th>';
            fwrite($myfile,$tag);
            $tag = '</tr>';
            fwrite($myfile,$tag);
            $tag = '</thead>';
            ...
            for($i=0;$i<$cnt;$i++)
            {
                ...
                fwrite($myfile,$tag);
                $tag = '<td>'.$returnedResult[$i]['stdcntint'].'</td>';
                fwrite($myfile,$tag);
                $tag = '<td>'.$returnedResult[$i]['stdcntext'].'</td>';
                fwrite($myfile,$tag);
                $tag = '</tr>';
                fwrite($myfile,$tag);
            }
